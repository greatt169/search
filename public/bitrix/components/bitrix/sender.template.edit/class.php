...
class SenderTemplateEditComponent extends CBitrixComponent
{
	...
	protected function preparePost()
	{
		...
		$content = Security\Sanitizer::cleanHtml($content);
		$content = Fileman\Block\EditorMail::removePhpFromHtml(
			$content,
			$this->entityTemplate->get('CONTENT'),
			Security\User::current()->canEditPhp(),
			Security\User::current()->canUseLpa()
		);
		$data = Array(
			"CONTENT"	=> $content,
